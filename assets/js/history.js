jQuery(document).ready(function ($) {
  ...
  $(document).on("click", ".view-details-button", function (e) {
    ...
    // Show loading state
    $modalBody.html(
      '<div class="loading">' + ArcheusBookingHistory.loading_text + "</div>"
    );
    $modal.show();
  ...
  function renderHistoryDetails(data) {
    ...
    html +=
      "<tr><th>" +
      archeus_booking_l10n.customer_name +
      "</th><td>" +
      escapeHtml(data.customer_name) +
      "</td></tr>";
    html +=
      "<tr><th>" +
      archeus_booking_l10n.customer_email +
      "</th><td>" +
      escapeHtml(data.customer_email) +
      "</td></tr>";
    html +=
      "<tr><th>" +
      archeus_booking_l10n.booking_date +
      "</th><td>" +
      escapeHtml(data.booking_date) +
      "</td></tr>";
    html +=
      "<tr><th>" +
      archeus_booking_l10n.booking_time +
      "</th><td>" +
      escapeHtml(data.booking_time) +
      "</td></tr>";
    html +=
      "<tr><th>" +
      archeus_booking_l10n.service_type +
      "</th><td>" +
      escapeHtml(data.service_type) +
      "</td></tr>";
    html +=
      "<tr><th>" +
      archeus_booking_l10n.price +
      "</th><td>" +
      (data.price
    ...
    html +=
      "<tr><th>" +
      archeus_booking_l10n.status +
      '</th><td><span class="status-badge status-' +
      data.status +
      '">' +
      escapeHtml(data.status.charAt(0).toUpperCase() + data.status.slice(1)) +
      "</span></td></tr>";
    ...
    html +=
      "<tr><th>" +
      archeus_booking_l10n.moved_at +
      "</th><td>" +
      escapeHtml(data.moved_at) +
      "</td></tr>";
    ...
    if (data.rejection_reason) {
      html +=
        "<tr><th>" +
        archeus_booking_l10n.rejection_reason +
        "</th><td>" +
  ...
  function formatFieldValue(key, value) {
    ...
    if (
      typeof value === "string" &&
      fileFields.some(function (field) {
        return (
          key.toLowerCase().includes(field) || field.includes(key.toLowerCase())
        );
      })
    ) {
  ...
  function exportHistoryToHTML() {
    ...
    $exportButton.html(
      '<span class="dashicons dashicons-update spinning"></span> Exporting...'
    );
